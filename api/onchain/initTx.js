...
const { buyerPubkey, sellerPubkey, amountUnits, expiresAt } = req.body||{};
if(!buyerPubkey||!sellerPubkey||!amountUnits||!expiresAt) return res.status(400).json({ ok:false, error:'Missing fields'});
...
const [vaultAuth] = vaultAuthPda(escrow, programId);
const program = new anchor.Program(require('../idl/artcoin_escrow.json'), programId, { connection });
const ix = await program.methods.initEscrow(new anchor.BN(amountUnits), new anchor.BN(expiresAt))
  .accounts({ payer: buyer, buyer, seller, usdtMint, escrow, vaultAuth, systemProgram: SystemProgram.programId }).instruction();
