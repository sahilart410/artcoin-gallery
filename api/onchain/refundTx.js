...
const ixRef = await program.methods.refund().accounts({ admin: null, escrow, usdtMint, buyerAta: ata, vaultAuth, vaultAta, tokenProgram: require('@solana/spl-token').TOKEN_PROGRAM_ID }).instruction();
const tx = await buildTx([...pre, ixRef], buyer, connection);
